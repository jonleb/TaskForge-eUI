...
Permission service is extending UserState with rights property. If You want to use Permissions Service, You should provide rights property into the User model that is [configured user detail end-point's](https://eui.ecdevops.eu/eui-showcase-ux-components-19.x/showcase-dev-guide/docs/10-other-resources/ngrx) response.
```javascript
export interface UserExtendedState extends UserState {  
    rights: Array<EuiUserRight>;  
}  
  
export interface EuiUserRight {
    id: string;
    permissions?: string[];
}
...
export class AppComponent {
	...
	// fetch the User rights either from calling Endpoint
	fetchUserRights() {
		...
	}
}
...
For the initialization example above you have to keep in mind that Initialization is Asynchronous and at that point Rendering of the application has already started. This means that by the time that Permission service will have initiated the rendering will have finished. In case there's somewhere a EuiHasPermissionDirective on the template that part will not be rendered until Permission information initiated.
We strongly suggest to use AppStarterService in order to initialize your service just after the user fetch.
EuiPermissionService is providing helper methods. These methods are used by **EuiHasPermissionDirective** into Eui-Core. But They are publicly open to using whenever you want in your application.
```javascript
export class EuiPermissionService extends EuiService<EuiUserRight[]> {
     ...
     */
    getState(): Observable<EuiUserRight[]>;
...
[//]: # (<em>**Experimental**</em>)
[//]: # (> <iframe width="100%" height="300px" src="https://stackblitz.com/edit/sb-permission-service-initiate?embed=1&file=src/app/app.component.ts&hideNavigation=1"></iframe>)
...
export class AuthGuardService implements CanActivate, CanActivateChild {
    ...
    constructor(private router: Router,
                ...
                private appShell: EuiAppShellService) {
    }
    ...
    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
        ...
        return this.appStarterService.onReady.pipe(filter(v=>v), map( () => {
            ...
            if (isLoggedIn) {
                ...
                if (this.euiPermissionService.checkRight(route.data.id)) {
                    ...
                    this.appShell.growlWarning('You don\'t have the permission access this page');
                }
            } else {
    
                // Giving feedback to user and redirecting here.
                this.appShell.growlWarning('You have to login to access this page');
...
[//]: # (TODO: Fix the stackblitz example)
[//]: # (<em>**Experimental**</em>)
[//]: # (> <iframe width="100%" height="300px" src="https://stackblitz.com/edit/sb-permission-service-authguard?embed=1&file=src/app/app.component.ts&hideNavigation=1"></iframe>)

### Permissions Directive
In case there are elements/components in your application that are visible to your users based on the permissions or Rights they have, you can use the EuiHasPermissionDirective. It is part of the **@eui/core** package. You can directly use it in a template. It will use the EuiPermissionsService data dynamically. As input, you can pass a String array (string formatted). Every value of that array can be either a Right or permission. Permission should be formatted like this, [**RIGHT_ID**] **dot** [**PERMISSION_ID**] (_e.g. RIGHT_ID.permission_ID_) . It can also be formated like this, [RIGHT_ID_1].[RIGHT_ID_2]....[RIGHT_ID_N].[PERMISSION_ID] (_e.g. RIGHT_.RIGHT_2.RIGHT_3.Perm_1_).
You should import EuiHasPermissionDirectiveModule in order to use this directive.
> *euiHasPermission=" [ 'SingleRight_Id','**Right_Id**.**PermissionId**' , 'JMS_MONITORING.filter' ]
```html
...
<div *euiHasPermission="['JMS_MONITORING.search','JMS_MONITORING.filter']">
...
import { AuthConfig, DEFAULT_AUTH_CONFIG } from './eui-auth.config';
...
EuiDynamicMenuService is a helper service that working with the Permission service and helps to load menuLinks in a dynamic way and filter them with user context. We have already defined the rights model. If You want to load your menu links from the server, You can add menuLinks property to the [configured user detail end-point's](https://eui.ecdevops.eu/eui-showcase-ux-components-19.x/showcase-dev-guide/docs/10-other-resources/ngrx) response.

### Data Model
The menuLinks should fit the [UxLinkLegacy](https://citnet.tech.ec.europa.eu/CITnet/stash/projects/CSDR/repos/eui/browse/packages/base/src/lib/ux-models/ux-link-legacy.model.ts) model. They will be used to generate a menu from that links.

If allowedRightId provided into ux-link-legacy, service will check it.
```javascript
{
   "user":{
      "userId":"",

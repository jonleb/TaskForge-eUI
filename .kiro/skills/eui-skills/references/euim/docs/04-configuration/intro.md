...
```javascript
import { BaseConfig } from '@eui/core';

const moduleConfig: ModulesConfig = {
	modules: {
    	moduleA: {
        	api: {
            	base: 'https://intragate.ec.europa.eu/my-app',
            	section1: '/section1',
            	section2: '/section2',
        	},
			logger: {
			...
			config: {
				configuration_to_initialize: own_service_or_own_modules
			}
    	}
	}
}
...
## Application State
An eUI application is using NgRx as state management tool. By default, the state is being retrieved and saved (synced) with localstorage. You can configure and change that to use SessionStorage instead.

---
description: Enable single selection so only one node can be selected at a time.
id: single-select
---

```html
<eui-tree [nodes]="treeData" isSingleSelect (selectionChange)="onSelectionChange($event)" />
```

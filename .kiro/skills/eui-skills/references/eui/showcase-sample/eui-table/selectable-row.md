---
description: Toggles built-in row selection with header and row directives.
id: selectable-row
---

```html
No selection feature <eui-slide-toggle (slideToggleChange)="onChangeSelectionFeature($event)" isChecked /> With selection feature

<div class="eui-table__scrollable-wrapper">
    <table #euiTable euiTable isTableResponsive [data]="data" [paginator]="paginator" (rowsSelect)="onRowsSelect($event)">
        ...
        <ng-template let-row euiTemplate="body">
            <tr [isDataSelectable]="hasSelectionFeature ? row : null">
...
@Component({
    selector: 'selectable-row',
    templateUrl: 'component.html',
    imports: [...EUI_TABLE, EuiTemplateDirective, ...EUI_SLIDE_TOGGLE, ...EUI_PAGINATOR, DecimalPipe],
...
export class SelectableRowComponent {
    ...
    public onChangeSelectionFeature(e: boolean): void {
        if (e) {

---
description: Template-driven dialog opened via ViewChild using eui-block-document to block the whole document during async validation.
id: with-eui-block-document
---

```html
<button euiButton type="button" (click)="openDialog()" aria-haspopup="dialog">Open Dialog</button>

<eui-dialog #dialog [title]="'Dialog with eui-block-document'" [hasClosedOnEscape]="false">
    <eui-block-document [isBlocked]="isBlocked" />
    <form [formGroup]="form">
        <div euiInputGroup>

...
export class IconSetsComponent implements OnInit {
    @ViewChild('popover') popover: EuiPopoverComponent;

    selectedIcon: string;
    selectedSet: 'eui' | 'ecl' | 'filled' | 'outline' = 'eui';
    selectedSize = 'm';
    ...
    "eui-download",
    "eui-edit",
    "eui-ellipsis-horizontal",
    "eui-ellipsis-vertical",
    "eui-email",
    "eui-expand",
    "eui-external-link",
    "eui-flash",
    ...
    "eui-menu",
    "eui-notifications",
    "eui-options-vertical",
    "eui-options",
    "eui-print",
    "eui-redo",
    "eui-refresh",
    "eui-remove",
    "eui-resize-horizontal",
    "eui-resize-vertical",
    "eui-search",
    "eui-send",
    "eui-settings",
    ...
    euiCoreRendered: string[] = [];
    euiFilesRendered: string[] = [];
    ECLRendered: string[] = [];
    phosphorRendered: string[] = [];

    private selectedIconSet = 'All';
    ...
    ngOnInit(): void {
        ...
        // Search filter init
        this.euiCoreRendered = this.eui;
        this.euiFilesRendered = this.euiFiles;
        this.ECLRendered = this.ECL;
        this.phosphorRendered = this.phosphor;

        this.searchFormValidated.get('searchFieldValidated').valueChanges.subscribe( resp => {
            if (resp) {
                this.euiCoreRendered = this.eui.filter(icon => icon.toLowerCase().indexOf(resp.toLowerCase()) !== -1);
                this.euiFilesRendered = this.euiFiles.filter(icon => icon.toLowerCase().indexOf(resp.toLowerCase()) !== -1);
                this.ECLRendered = this.ECL.filter(icon => icon.toLowerCase().indexOf(resp.toLowerCase()) !== -1);
                this.phosphorRendered = this.phosphor.filter(icon => icon.toLowerCase().indexOf(resp.toLowerCase()) !== -1);
            } else {
                this.euiCoreRendered = this.eui;
                this.euiFilesRendered = this.euiFiles;
                this.ECLRendered = this.ECL;
    ...
    public onIconSetSelect(value: Event) {
        this.selectedIconSet = value.target['value'];
    }

    public onSearchValidated() {
        let resp = this.searchFormValidated.get('searchFieldValidated').value;
        if (resp) {
            this.growlService.growl({
                severity: 'info', summary: 'Search',
                detail: `search: ${this.searchFormValidated.get('searchFieldValidated').value}, iconSet: ${this.selectedIconSet}`,
            });
        } else {
            this.growlService.growl({
                severity: 'warning', summary: 'Search',
                detail: `Please type the icon name/label to search for.`,
            });
        }
    }
}

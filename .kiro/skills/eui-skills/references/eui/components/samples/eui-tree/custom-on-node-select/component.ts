...
import { TreeDataModel, EuiTreeSelectionChanges, CustomNodeSelectFn, EUI_TREE } from "@eui/components/eui-tree";
import { EUI_FIELDSET } from '@eui/components/eui-fieldset';


const customFn: CustomNodeSelectFn = (path,isChecked, treeItem, helper) => {
    helper.select(path, isChecked);
    helper.getParents(path).forEach((parentPath)=>{
        if(isChecked){
            helper.select(parentPath, isChecked);
        } else{
            const isAllChildrenNotSelected = helper.getTreeItem(parentPath)?.children.every((item)=>{
                return !item.node.isSelected;
            });
            if(isAllChildrenNotSelected){
                helper.select(parentPath, false);
            }
        }
    });
}
...
@Component({
    ...
    selector: 'Custom-on-node-select',
    templateUrl: 'component.html',
    imports: [...EUI_TREE, ...EUI_INPUT_CHECKBOX, ...EUI_FIELDSET],
})
export class CustomOnNodeSelectComponent {

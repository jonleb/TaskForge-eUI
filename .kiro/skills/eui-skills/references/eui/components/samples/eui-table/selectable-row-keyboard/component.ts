...
@Component({
    selector: 'selectable-row-keyboard',
    templateUrl: 'component.html',
    imports: [...EUI_TABLE, ...EUI_PAGINATOR, ...EUI_ALERT, EuiTemplateDirective, EuiTooltipDirective, ...EUI_DROPDOWN, ...EUI_ICON, ...EUI_SLIDE_TOGGLE, ...EUI_LIST, ...EUI_ICON_BUTTON],
    providers: [EuiGrowlService],
})
export class SelectableRowKeyboardComponent {
    ...
    constructor(private euiGrowlService: EuiGrowlService) {}

    public onRowsSelect(selected: Country[]): void {
        console.log(selected);
    }

    public onSlideToggleClick(e: any) {
        console.log(e);
        e.stopPropagation();
    }
    ...
    public onRowEdit(row: Country, e: Event) {
        this.euiGrowlService.growlInfo(JSON.stringify(row));
        e.stopPropagation();
    }

    public onRowDetail(row: Country, e: Event) {
        this.euiGrowlService.growlInfo(JSON.stringify(row));
        e.stopPropagation();
    }

    public onRowDelete(row: Country, e: Event) {
        this.euiGrowlService.growlError(JSON.stringify(row));
        e.stopPropagation();
    }
